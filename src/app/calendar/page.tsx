...
export default async function CalendarPage({ searchParams }: { searchParams: Promise<{ event?: string; date?: string; view?: string }> }) {
  ...
  const [ctx, cal, people] = await Promise.all([frameContext(user), getCalendar(viewer, fromISO, toISO), getPeople().catch(() => [])]);
  return (
    <AppFrame user={ctx}>
      <Suspense fallback={null}>
        <CalendarClient configured={cal.configured} items={cal.items} feeds={cal.feeds} people={people.map((p) => ({ id: p.id, name: p.name, greetingName: p.greetingName, hue: p.hue, avatarUrl: p.avatarUrl, kind: p.kind }))} todayISO={todayISO} fromISO={todayISO} toISO={addDaysISO(todayISO, 30)} viewer={viewer} openEventId={event && /^\d+$/.test(event) ? Number(event) : null} initialDate={date && /^\d{4}-\d{2}-\d{2}$/.test(date) ? date : null} initialView={view === "week" || view === "month" ? view : "agenda"} />
      </Suspense>
    </AppFrame>
  );
}
