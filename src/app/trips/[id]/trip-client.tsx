...
function Inner({ trip, people, me, isOwner, recentPhotos, initialTab }: { trip: TripDetail; people: PersonLite[]; me: string | null; isOwner: boolean; recentPhotos: Photo[]; initialTab: string }) {
  ...
  return (
    <div style={{ width: "100%", maxWidth: "var(--content-max)", margin: "0 auto", padding: "8px var(--page-gutter-mobile) 64px", display: "flex", flexDirection: "column", gap: "var(--space-8)" }}>
      <Reveal>
        ...
        <PhotoHero src={trip.cover} ratio="21 / 9" eyebrow={tripDates(trip)} title={trip.name} subtitle={[trip.destination, countdownLabel(trip.countdown)].filter(Boolean).join(" · ")} emptyTitle={trip.name} emptyBody={[trip.destination, tripDates(trip), countdownLabel(trip.countdown)].filter(Boolean).join(" · ") || "Add a cover photo from the library."} emptyAction={canEdit ? <Button variant="secondary" size="sm" iconLeft="image" onClick={() => setCover(true)}>Choose a cover</Button> : <span />} topRight={canEdit ? <><IconButton icon="image" label="Change cover" variant="onPhoto" size="sm" onClick={() => setCover(true)} /><IconButton icon="settings" label="Trip settings" variant="onPhoto" size="sm" onClick={() => setSettings(true)} /></> : undefined} />
