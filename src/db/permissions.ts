...
export async function listAccountAccess(): Promise<AccountAccessRow[]> {
...
export async function getModuleAccess(memberId: string): Promise<Record<string, boolean>> {
  if (!isDbConfigured || !db) return {};
...
export async function listModuleAccess(): Promise<{ memberId: string; module: string; allowed: boolean }[]> {
  if (!isDbConfigured || !db) return [];
...
export async function listMembersAdmin(): Promise<MemberAdmin[]> {
  if (!isDbConfigured || !db) return [];
...
export async function listHouseholdAccounts(): Promise<HouseholdAccount[]> {
  if (!isDbConfigured || !db) return [];
