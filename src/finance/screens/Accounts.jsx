...
/* ---- account detail ------------------------------------------------------
   Everything edits INLINE — click a value, change it, save. No trip through a
   settings modal. Activity below is searchable, filterable, and its category
   chips are tappable (same learn-as-you-fix behavior as Transactions). */

function ZHQInlineRow({ label, sub, display, value, type = 'text', options, onSave, last }) {
  const { Icon, TextInput, Select, Button } = window.ZittingHQDesignSystem_c9e528;
  const [editing, setEditing] = React.useState(false);
  const [val, setVal] = React.useState(value ?? '');
  const [busy, setBusy] = React.useState(false);
  async function save() {
    setBusy(true);
    try { await onSave(val); setEditing(false); window.ZHQ_REFRESH && window.ZHQ_REFRESH(); }
    finally { setBusy(false); }
  }
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '10px 0', borderBottom: last ? 'none' : '1px solid var(--border-hairline)', minHeight: 46 }}>
      <div style={{ minWidth: 0, flex: 'none' }}>
        <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>{label}</div>
        {sub ? <div style={{ fontSize: 11, color: 'var(--text-tertiary)', marginTop: 2 }}>{sub}</div> : null}
      </div>
      {editing ? (
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, flex: 1, justifyContent: 'flex-end', minWidth: 0 }}>
          {type === 'select' ? (
            <Select value={val} onChange={setVal} options={options} style={{ minWidth: 150 }} />
          ) : (
            <TextInput value={val} onChange={setVal} style={{ width: 160 }} onKeyDown={(e) => { if (e.key === 'Enter') save(); if (e.key === 'Escape') setEditing(false); }} autoFocus />
          )}
          <Button variant="primary" size="sm" onClick={save} disabled={busy}>{busy ? '…' : 'Save'}</Button>
          <Button variant="ghost" size="sm" onClick={() => { setVal(value ?? ''); setEditing(false); }} disabled={busy}>Cancel</Button>
        </div>
      ) : (
        <button onClick={() => { setVal(value ?? ''); setEditing(true); }} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'none', border: 'none', cursor: 'pointer', fontSize: 13.5, color: 'var(--text-primary)', fontWeight: 500, font: 'inherit', minWidth: 0, minHeight: 36 }}>
          <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 220 }}>{display ?? (value || '—')}</span>
          <Icon name="pencil" size={13} style={{ color: 'var(--text-tertiary)', flex: 'none' }} />
        </button>
      )}
    </div>
  );
...
function ZHQManagersRow({ acct }) {
  const { Icon, Checkbox, Avatar } = window.ZittingHQDesignSystem_c9e528;
  const API = window.ZHQ_API || {};
  const members = window.ZHQ_DATA.members || [];
  const [open, setOpen] = React.useState(false);
  const [busy, setBusy] = React.useState(false);
  const managerIds = (acct.managers || []).map((m) => m.id);
  async function toggle(id) {
    const next = managerIds.includes(id) ? managerIds.filter((x) => x !== id) : (managerIds.length >= 2 ? managerIds : [...managerIds, id]);
    if (next.join() === managerIds.join() || !API.setAccountMembers) return;
    setBusy(true);
    try { await API.setAccountMembers(acct.id, next); window.ZHQ_REFRESH && window.ZHQ_REFRESH(); } finally { setBusy(false); }
  }
  return (
    <div style={{ padding: '10px 0', borderBottom: '1px solid var(--border-hairline)' }}>
      <button onClick={() => setOpen((v) => !v)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, width: '100%', background: 'none', border: 'none', cursor: 'pointer', font: 'inherit', padding: 0, minHeight: 36 }}>
        <div>
          <div style={{ fontSize: 13, color: 'var(--text-secondary)', textAlign: 'left' }}>In charge of</div>
          <div style={{ fontSize: 11, color: 'var(--text-tertiary)', marginTop: 2, textAlign: 'left' }}>Up to 2 people categorize this account from their login</div>
        </div>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8, fontSize: 13.5, color: 'var(--text-primary)', fontWeight: 500 }}>
          {(acct.managers || []).map((m) => m.name).join(', ') || 'No one yet'}
          <Icon name={open ? 'chevronDown' : 'chevronRight'} size={14} style={{ color: 'var(--text-tertiary)' }} />
        </span>
      </button>
      {open ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2, paddingTop: 8, opacity: busy ? 0.6 : 1 }}>
          {members.length ? members.map((mem) => {
            const checked = managerIds.includes(mem.id);
            const atCap = managerIds.length >= 2 && !checked;
            return (
              <label key={mem.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 2px', cursor: atCap || busy ? 'not-allowed' : 'pointer', opacity: atCap ? 0.45 : 1 }}>
                <Checkbox checked={checked} onChange={() => toggle(mem.id)} disabled={atCap || busy} />
                <Avatar name={mem.name} size="xs" />
                <span style={{ fontSize: 13.5, color: 'var(--text-primary)' }}>{mem.name}</span>
              </label>
            );
          }) : <div style={{ fontSize: 12.5, color: 'var(--text-tertiary)' }}>Add people in Settings → Access first.</div>}
        </div>
      ) : null}
    </div>
  );
...
function ZHQAccountDetail({ acct, onBack }) {
  const { Card, Button, Icon, Badge, AreaChart, DataTable, AmountCell, Tag, SegmentedControl, Modal } = window.ZittingHQDesignSystem_c9e528;
  ...
  // --- activity: search + scope + show more + tappable category chips ---
  const [q, setQ] = React.useState('');
  const [scope, setScope] = React.useState('All');
  const [limit, setLimit] = React.useState(12);
  const [catPick, setCatPick] = React.useState(null); // txn id
  const [busyTxn, setBusyTxn] = React.useState(null);
  const [confirmDelete, setConfirmDelete] = React.useState(false);
  const [busy, setBusy] = React.useState(false);

  const allRows = (D.txns || []).filter((t) => t.accountId === acct.id || (acct.mask && t.account && t.account.includes(acct.mask)));
  const scoped = scope === 'Spending' ? allRows.filter((t) => !t.income && !t.isTransfer)
    : scope === 'Income' ? allRows.filter((t) => t.income && !t.isTransfer)
    : scope === 'Needs review' ? allRows.filter((t) => !t.reviewed)
    : allRows;
  ...
  async function setCategory(id, categoryId) {
    setBusyTxn(id);
    try { await API.updateTransaction(id, { categoryId }, { learn: true }); window.ZHQ_REFRESH && window.ZHQ_REFRESH(); }
    finally { setBusyTxn(null); setCatPick(null); }
  }
  async function changeVis(mode) {
    ...
    try {
      ...
      window.ZHQ_REFRESH && window.ZHQ_REFRESH();
      if (mode === 'hidden') onBack();
    } finally { setBusy(false); }
  }
  async function removeAccount() {
    if (!API.deleteAccount) return;
    setBusy(true);
    try { await API.deleteAccount(acct.id); window.ZHQ_REFRESH && window.ZHQ_REFRESH(); onBack(); } finally { setBusy(false); }
  }

  // This-month quick stats for THIS account (from its rows).
  const monthName = new Date().toLocaleString('en-US', { month: 'long' });
  let mIn = 0, mOut = 0;
  for (const t of allRows) {
    if (t.isTransfer) continue;
    if (t.income) mIn += t.amt; else if (t.amt < 0) mOut += -t.amt; else mOut -= t.amt;
  }

  const patch = (p) => API.updateAccount(acct.id, p);
  const catChip = (r) => (
    <button data-no-drag onClick={() => setCatPick(r.id)} disabled={busyTxn === r.id}
      style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', opacity: busyTxn === r.id ? 0.5 : 1 }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      <button onClick={onBack} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, alignSelf: 'flex-start', background: 'none', border: 'none', color: 'var(--text-secondary)', font: 'inherit', fontSize: 13, cursor: 'pointer', minHeight: 36 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'var(--grid-2)', gap: 16 }}>
        <Card padding={24}>
          ...
          <div style={{ display: 'flex', gap: 26, marginTop: 16, paddingTop: 14, borderTop: '1px solid var(--border-hairline)' }}>
            <div>
              <div className="zt-eyebrow" style={{ marginBottom: 4 }}>In · {monthName}</div>
              <div className="zt-num" style={{ fontSize: 17, fontWeight: 600, color: 'var(--accent)' }}>{ZHQMoney(mIn, false)}</div>
            </div>
            <div>
              <div className="zt-eyebrow" style={{ marginBottom: 4 }}>Out · {monthName}</div>
              <div className="zt-num" style={{ fontSize: 17, fontWeight: 600, color: 'var(--text-primary)' }}>{ZHQMoney(mOut, false)}</div>
            </div>
            <div>
              <div className="zt-eyebrow" style={{ marginBottom: 4 }}>Activity</div>
              <div className="zt-num" style={{ fontSize: 17, fontWeight: 600, color: 'var(--text-primary)' }}>{allRows.length}</div>
            </div>
          </div>
          <div style={{ marginTop: 16 }}>
            <AreaChart data={acct.trend} labels={[]} height={130} color={credit ? 'var(--gray-400)' : 'var(--accent)'} />
          </div>
        </Card>

        <Card padding={22}>
          <span className="zt-eyebrow" style={{ display: 'block', marginBottom: 6 }}>Account settings · click any value to edit</span>
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            <ZHQInlineRow label="Nickname" value={acct.name} onSave={(v) => v.trim() && patch({ name: v.trim() })} />
            <ZHQInlineRow label="Institution" value={acct.inst} onSave={(v) => patch({ institution: v.trim() })} />
            <ZHQInlineRow label="Last 4 (mask)" value={acct.mask || ''} onSave={(v) => patch({ mask: v.trim() || null })} />
            <ZHQInlineRow label="Type" type="select" value={acct.type}
              display={acct.type === 'credit' ? 'Credit card' : acct.type === 'savings' ? 'Savings' : 'Checking'}
              options={[{ value: 'checking', label: 'Checking' }, { value: 'savings', label: 'Savings' }, { value: 'credit', label: 'Credit card' }]}
              onSave={(v) => patch({ type: v })} />
            <ZHQInlineRow label="Mapped to" type="select" value={acct.who}
              options={[{ value: 'Household', label: 'Household' }, ...members.map((m) => ({ value: m.name, label: m.name }))]}
              onSave={(v) => patch({ who: v })} />
            <ZHQManagersRow acct={acct} />
            <ZHQInlineRow label="Transfer destination" sub="Optional tag for allocation targets" value={acct.dest || ''} display={acct.dest || 'None'} onSave={(v) => patch({ destLabel: v.trim() || null })} />
            <ZHQOpeningBalanceRow acct={acct} />
            <div style={{ padding: '12px 0 4px' }}>
              <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 8 }}>Visibility</div>
              <SegmentedControl full value={acct.collapsed ? 'grouped' : 'shown'} onChange={changeVis}
                options={[{ value: 'shown', label: 'Shown' }, { value: 'grouped', label: 'Grouped' }, { value: 'hidden', label: 'Hidden' }]} />
              <div style={{ fontSize: 11.5, color: 'var(--text-tertiary)', marginTop: 7, lineHeight: 1.5 }}>
                Grouped tucks it into the &ldquo;Other accounts&rdquo; card; Hidden removes it from the dashboard, emails, and sync.
              </div>
            </div>
            <div style={{ paddingTop: 10, borderTop: '1px solid var(--border-hairline)', marginTop: 6 }}>
              {confirmDelete ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                  <span style={{ fontSize: 12.5, color: 'var(--text-secondary)', flex: 1, minWidth: 160 }}>Delete this account and its imported transactions? This can&apos;t be undone.</span>
                  <Button variant="ghost" size="sm" onClick={() => setConfirmDelete(false)} disabled={busy}>Keep</Button>
                  <Button variant="destructive" size="sm" onClick={removeAccount} disabled={busy}>{busy ? '…' : 'Delete account'}</Button>
                </div>
              ) : (
                <button onClick={() => setConfirmDelete(true)} style={{ background: 'none', border: 'none', cursor: 'pointer', font: 'inherit', fontSize: 12.5, color: 'var(--negative)', padding: '6px 0' }}>Delete account…</button>
              )}
            </div>
          </div>
        </Card>
      ...
      <Card>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', marginBottom: 14 }}>
          <h2 style={{ fontSize: 17, fontWeight: 600, color: 'var(--text-primary)' }}>Activity</h2>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, height: 36, padding: '0 12px', background: 'var(--surface-sunken)', border: '1px solid var(--border-hairline)', borderRadius: 'var(--radius-pill)', color: 'var(--text-tertiary)', flex: '1 1 160px', minWidth: 140, maxWidth: 280 }}>
            <Icon name="search" size={14} style={{ flex: 'none' }} />
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search this account…" aria-label="Search this account's activity"
              style={{ flex: 1, minWidth: 0, background: 'none', border: 'none', outline: 'none', font: 'inherit', fontSize: 13, color: 'var(--text-primary)' }} />
          </div>
          <div className="zhq-hscroll" style={{ overflowX: 'auto', marginLeft: 'auto' }}>
            <SegmentedControl options={['All', 'Spending', 'Income', 'Needs review']} value={scope} onChange={(v) => { setScope(v); setLimit(12); }} size="sm" />
          </div>
        </div>

        {rows.length === 0 ? (
          <div style={{ padding: '18px 4px', fontSize: 13.5, color: 'var(--text-tertiary)' }}>
            {needle ? `Nothing matches \u201C${q.trim()}\u201D here.` : 'No transactions on this account yet.'}
          </div>
        ) : (
          <>
            ...
            <div className="zhq-mobile-block">
              {rows.map((r, i) => (
                <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '10px 2px', borderTop: i ? '1px solid var(--border-hairline)' : 'none' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.merchant}</div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginTop: 4 }}>
                      {catChip(r)}
                      <span className="zt-num" style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>{r.date}</span>
                    </div>
                  </div>
            ...
            <div className="zhq-desktop-only">
              <DataTable
                columns={[
                  ...
                  { key: 'cat', header: 'Category', render: (r) => catChip(r) },
                  { key: 'who', header: 'Person', render: (r) => <span style={{ color: 'var(--text-secondary)', fontSize: 13 }}>{r.who}</span> },
            ...
            {filtered.length > limit ? (
              <div style={{ display: 'flex', justifyContent: 'center', paddingTop: 12 }}>
                <Button variant="ghost" size="sm" onClick={() => setLimit((l) => l + 25)}>Show more · {filtered.length - limit} left</Button>
              </div>
            ) : null}
          </>
        )}
      ...
      <Modal open={catPick != null} onClose={() => setCatPick(null)} title="Choose category" width={360}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2, maxHeight: 360, overflowY: 'auto' }}>
          {cats.map((c) => (
            <button key={c.id} onClick={() => setCategory(catPick, c.id)} className="zhq-nav-item" data-active="false" style={{ width: '100%' }}>
