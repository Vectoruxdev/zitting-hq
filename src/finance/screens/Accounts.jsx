...
function ZHQAccountDetail({ acct, onBack }) {
  ...
  const credit = acct.balance < 0;
  const rows = D.txns.filter((t) => t.account.includes(acct.mask)).concat(D.txns).slice(0, 5);
  return (
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 16 }}>
        ...
        <Button variant="primary" iconLeft={<Icon name="plus" size={16} />} style={{ flex: 'none' }}>Connect account</Button>
      </div>
      ...
      {groups.map(([title, list]) => (
        <div key={title}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14 }}>
            ...
            {title === 'Checking' ? (
              <button onClick={() => {}} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8, minHeight: 132, border: '1px dashed var(--border-strong)', borderRadius: 'var(--radius-lg)', background: 'transparent', color: 'var(--text-secondary)', cursor: 'pointer', font: 'inherit' }}>
                <Icon name="plus" size={20} /><span style={{ fontSize: 13, fontWeight: 500 }}>Connect account</span>
              </button>
            ) : null}
          </div>
        </div>
