  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', display: 'flex', flexDirection: 'column', height: 'calc(100vh - 160px)' }}>
      <div style={{ textAlign: 'center', padding: '6px 0 22px' }}>
        ...
        <p style={{ fontSize: 13.5, color: 'var(--text-secondary)', marginTop: 6 }}>Your private family coach. It can read budgets, transactions, and goals.</p>
      </div>

      <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 16, padding: '4px 2px' }}>
        {msgs.map((m, i) => m.role === 'user' ? (
          ...
          <div key={i} style={{ alignSelf: 'flex-start', maxWidth: '88%', display: 'flex', gap: 11 }}>
            ...
            <div>
              <div style={{ background: 'var(--surface-card)', padding: '12px 15px', borderRadius: '4px 16px 16px 16px', fontSize: 14, color: 'var(--paper-200)', lineHeight: 1.55 }}>{m.text}</div>
              {m.chart === 'dining' ? (
      ...
      <div style={{ paddingTop: 14 }}>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
          {D.ask.prompts.map((p) => (
            <button key={p} onClick={() => send(p)} style={{ fontSize: 12.5, color: 'var(--text-secondary)', background: 'var(--surface-raised)', border: '1px solid var(--border-hairline)', borderRadius: 'var(--radius-pill)', padding: '7px 13px', cursor: 'pointer' }}>{p}</button>
          ))}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 8px 8px 18px', background: 'var(--surface-card)', border: '1px solid var(--border-hairline)', borderRadius: 'var(--radius-pill)' }}>
          <input value={draft} onChange={(e) => setDraft(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') send(draft); }} placeholder="Ask about spending, budgets, tithing…" style={{ flex: 1, background: 'none', border: 'none', outline: 'none', color: 'var(--text-primary)', fontSize: 14 }} />
          <Button variant="accent" size="sm" onClick={() => send(draft)} iconRight={<Icon name="arrowRight" size={15} />}>Ask</Button>
        </div>
      </div>
    </div>
  );
